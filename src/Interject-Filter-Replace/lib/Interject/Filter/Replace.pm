package Interject::Filter::Replace;
...
# This allows declaration	use Interject::Filter::Replace ':all';
...
sub apply {
	...
	if (($self->{'filterdirection'} eq $self->SUPER::BOTH) || ($self->{'filterdirection'} eq $filterdirection)) {
		$inputstring =~ s/$self->{'targetpattern'}/$self->{'replacementstring'}/g;
	}
	return $inputstring;
}
...
Interject::Filter::Replace - Perl extension for Interject to replace the input
...
=item Interject::Filter::Replace->new(<filtertdirection>, <targetpattern>, <replacementstring>)

Constructs a new C<Interject::Filter::Replace> object describing the replacement.

=item Interject::Filter::Replace->apply(<filterdirection>, <remotehostname>, <remoteportnumber>, <clienthostname>, <clientportnumber>, <inputstring>)
